import gradio as gr
from orchestrator_app_backend import graph
import uuid
...
def generate_essay(
    ...
    for s in graph.stream(initial_state, thread_config):
        ...
        # Adiciona uma lista divisória para separar os passos
        full_output += "---" * 20 + '\n\n'
...
with gr.Blocks(theme = gr.themes.Default(spacing_size='sm', text_size='sm')) as demo:
    ...
    with gr.Row():
        ...
        max_revisions_slider = gr.Slider(
            ...
            step=1,
            value=1,
            label="Número Máximo de Revisões"
        )
    ...
    output_texbox = gr.Textbox(
        label="Processo e Redação Final",
        lines=20,
        max_lines=40,
    )
